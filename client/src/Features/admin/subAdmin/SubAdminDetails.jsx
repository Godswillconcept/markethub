...
import { useSubAdminDetails } from "./useSubAdminDetails";
...
const SubAdminDetails = () => {
  ...
  const { subAdmin, isLoading, error } = useSubAdminDetails();
  const permissions = subAdmin?.roles?.[0]?.permissions || [];
  ...
  const displayResources = uniqueResources.map((r) => resourceLabels[r]);

  // console.log(displayResources);

  if (isLoading) return <Spinner />;
  if (!subAdmin) return <div className="p-4">Sub Admin not found</div>;
      ...
      <div className="rounded border border-gray-200">
        <div className="relative flex flex-col items-start space-y-6 rounded-lg bg-white p-6 lg:flex-row lg:items-center lg:space-y-0 lg:space-x-6 lg:p-4">
          <img
            ...
            src={getImageUrl(subAdmin.avatar)}
            alt={subAdmin.name}
          />
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-3 md:grid-cols-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-gray-700">
                  Date Joined: {formatDate(subAdmin.created_at)}
                </span>
              </div>
              <div className="flex items-center space-x-2">
                <PhoneIcon className="h-4 w-4 text-gray-500" />
                <span className="text-sm text-gray-700">{subAdmin.phone}</span>
              </div>
              <div className="flex items-center space-x-2">
                <UsersIcon className="h-4 w-4 text-gray-500" />
                <span className="text-sm text-gray-700">
                  Permissions: {displayResources.join(", ")}
                </span>
              </div>
            </div>
          </div>
