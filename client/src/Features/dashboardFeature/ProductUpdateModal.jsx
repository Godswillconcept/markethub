...
function ProductUpdateModal({
    ...
    useEffect(() => {
        // Removed console logging for production
    }, [currentRating, currentTitle, currentComment, isDirty, errors]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            await createReviewMutation.mutateAsync(reviewDataToSubmit);
    ...
    useEffect(() => {
        // Removed console logging for production
    }, [createReviewMutation.isPending, isSubmitting]);
    ...
    return (
        <Dialog open={isOpen} onClose={handleClose} className="relative z-50">
            ...
            <div className="fixed inset-0 flex items-center justify-center p-4">
                <DialogPanel className="w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-2xl bg-white shadow-2xl transform transition-all">
                    {/* Header */}
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 px-6 py-6">
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-sm text-gray-600 mt-1">
                                            Sold by: {vendorData.store?.business_name || vendorData.name || "Unknown Vendor"}
                                        </p>
                                        <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-gray-600">
                                            ...
                                            <span className="flex items-center gap-1">
                                                <span className="font-medium">Price:</span> ${typeof product.price === 'number' ? product.price.toFixed(2) : (product.price || "N/A")}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="flex items-center gap-1">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button
                                                ...
                                                className="text-gray-300 transition-all hover:scale-110 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-2 rounded"
                                            >
                                                {displayRating >= star ? (
                                    ...
                                    <span className="text-lg font-medium text-gray-700">
                                        {currentRating > 0 ? (
                                            <span className="text-yellow-600">{currentRating} out of 5</span>
                                        ) : (
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className={`w-full px-4 py-3 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow ${errors.title ? "border-red-300" : "border-gray-300"
                                        }`}
                                    {...register("title", {
                            ...
                            {/* Review Comment */}
                            <div className="space-y-2">
                                <label htmlFor="comment" className="block text-base font-semibold text-gray-900">
                                    Your Review <span className="text-red-500">*</span>
                                </label>
                                <textarea
                                    id="comment"
                                    placeholder="Share your experience with this product. What did you like or dislike?"
                                    className={`w-full px-4 py-3 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none transition-shadow ${errors.comment ? "border-red-300" : "border-gray-300"
                                        }`}
                                    rows={6}
                                    {...register("comment", {
                            ...
                            <div className="bg-blue-50 border-l-4 border-blue-400 rounded-r-lg p-4">
                                ...
                                <ul className="text-sm text-blue-800 space-y-1.5">
                                    ...
                                    <li className="flex items-start gap-2">
                                        <span className="text-blue-600 mt-0.5">•</span>
                                        <span>Focus on your personal experience with the product</span>
                                    </li>
                                    <li className="flex items-start gap-2">
                                        <span className="text-blue-600 mt-0.5">•</span>
                                        <span>Avoid offensive language or personal information</span>
                                    </li>
                                    <li className="flex items-start gap-2">
                                        <span className="text-blue-600 mt-0.5">•</span>
                    ...
                    <div className="sticky bottom-0 flex justify-end gap-3 border-t bg-gray-50 px-6 py-4">
                        ...
                        <button
                            ...
                            className="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg shadow-blue-500/30"
                        >
                            {isSubmitting && (
