  ...
  try {
    ...
    if (status && validStatuses.includes(status)) {
      if (status === "deactivated") {
        // Deactivated: Store check (unverified or inactive)
        ...
        // Suspended: Approved vendor but store issue
    ...
    if (search) {
      ...
      whereClause[Op.or] = [
        { '$store.business_name$': { [Op.like]: searchPattern } },
        { '$User.first_name$': { [Op.like]: searchPattern } },
        { '$User.last_name$': { [Op.like]: searchPattern } },
    ...
    const { count, rows: vendors } = await Vendor.findAndCountAll({
      ...
      include: [
        ...
        },
        {
          ...
          required: Object.keys(storeWhereClause).length > 0,
        },
      ],
      limit: parseInt(limit),
      offset: parseInt(offset),
      order: [["created_at", "DESC"]],
      // subQuery: false is critical here for performance and correctness
  ...
  try {
    ...
    try {
      await sendEmail(vendor.User.email, "VENDOR_REJECTED", {
        name: `${vendor.User.first_name} ${vendor.User.last_name}`,
        storeName: vendor.store.business_name,
        reason: "Your vendor application has been rejected. Please contact Stylay support for more details.",
        supportEmail: process.env.SUPPORT_EMAIL || "support@stylay.ng",
 ...
 * @param {import('express').Request} req - Express request object
 * @param {Object} req.body - Request body containing business information
 * @param {string} req.body.business_name - Business/store name (required)
 * @param {string} req.body.join_reason - Reason for joining as vendor (required)
 * @param {string} [req.body.cac_number] - CAC registration number (optional)
 * @param {string} [req.body.instagram_handle] - Instagram handle (optional)
 * @param {string} [req.body.facebook_handle] - Facebook handle (optional)
 * @param {string} [req.body.twitter_handle] - Twitter handle (optional)
 * @param {Object} req.user - Authenticated user info
 * @param {number} req.user.id - User ID for vendor lookup
 * @param {import('express').Response} res - Express response object
 * @param {import('express').NextFunction} next - Express next middleware function
 * @returns {Object} Success response with initialization confirmation
 * @returns {boolean} status - Success status
 * @returns {string} message - Success message
 * @returns {Object} data - Vendor and store initialization details
 * @returns {Object} data.vendor - Created vendor information
 * @returns {Object} data.store - Created store information
 * @throws {AppError} 400 - User already has vendor records or lacks vendor role
 * @throws {AppError} 404 - User not found
 * @throws {AppError} 500 - Server error during initialization
 * @api {post} /api/v1/vendors/initialize-onboarding Initialize Vendor Onboarding
 * @private vendor
 * @example
 * // Request
 * POST /api/v1/vendors/initialize-onboarding
 * Authorization: Bearer <token>
 * {
 *   "business_name": "My Awesome Store",
 *   "join_reason": "I want to sell handmade crafts",
 *   "cac_number": "RC/1234567"
 * }
 *
 * // Success Response (200)
    ...
    res.status(200).json({
      ...
      data: {
        vendor: {
          ...
          join_reason: newVendor.join_reason,
        },
        store: {
          id: newStore.id,
          business_name: newStore.business_name,
          slug: newStore.slug,
          cac_number: newStore.cac_number,
        },
      },
    });
  } catch (error) {
...
const updateVendorProfile = async (req, res, next) => {
  ...
  try {
    const vendorId = req.user.id;
    const {
        first_name, last_name, phone, // User fields
        business_name, description, cac_number, 
    ...
    if (first_name || last_name || phone) {
        await vendor.User.update({
            first_name: first_name || vendor.User.first_name,
            last_name: last_name || vendor.User.last_name,
    ...
    if (req.processedFiles) {
        if (req.processedFiles.logo) {
            storeUpdateData.pending_logo = req.processedFiles.logo;
        }
        if (req.processedFiles.business_images && Array.isArray(req.processedFiles.business_images)) {
    ...
    res.status(200).json({
        ...
        message: req.processedFiles?.logo 
            ? "Vendor profile updated successfully. Logo update is pending admin approval." 
 ...
 * @param {import('express').NextFunction} next - Express next middleware function
 */
const approveVendorLogo = async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    logger.error("Approve vendor logo error:", error);
    next(error);
  }
};
 ...
 * @param {import('express').NextFunction} next - Express next middleware function
 */
const rejectVendorLogo = async (req, res, next) => {
  try {
    const { id } = req.params;
    const vendor = await Vendor.findByPk(id, {
        include: [{ model: Store, as: 'store' }]
    ...
    logger.error("Reject vendor logo error:", error);
    next(error);
  }
};
