...
async function convertCartToOrder(cartSummary, options = {}) {
  try {
    ...
    for (const cartItem of cart.items) {
      ...
      // Check inventory using VariantCombination
...
async function createOrder(req, res) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (item.selected_variants && item.selected_variants.length > 0) {
        ...
        if (combinations.length === 0) {
          ...
          throw new Error(
            `Please select a variant for product: ${product.name}`
          );
        }

        const defaultCombination = combinations[0];
        if (defaultCombination.stock < item.quantity) {
          throw new Error(
            `Insufficient stock for product: ${product.name}. Available: ${defaultCombination.stock}`
          );
        }

        // Set combination info so stock is decremented later
        // Use variant price from combination if it differs? Usually combination has modifier.
        // But for simple product, we used product price.
        // We'll keep product price logic but attach combinationId.
        item.combinationId = defaultCombination.id;
      }
    ...
    let order = await Order.create(
      {
        user_id: userId,
        order_number: `TEMP-${uuidv4()}`, // Temporary order number to satisfy notNull constraint
    ...
    for (const item of itemsWithDetails) {
      ...
      if (item.combinationId || item.combination_id) {
        ...
        const combination = await sequelize.models.VariantCombination.findByPk(
          combinationId,
          { transaction }
        );
        if (!combination || combination.product_id !== product.id) {
          throw new Error(
            `Variant combination ${combinationId} not found for product ${product.id}`
          );
        }
        if (combination.stock < item.quantity) {
          throw new Error(
            `Insufficient stock for combination ${combinationId}`
          );
        }
...
async function getOrder(req, res) {
  try {
              ...
              include: [
                {
                  model: Store,
                  as: "store",
                  attributes: ["id", "business_name", "slug", "logo"],
                },
              ],
            },
          ],
    ...
    const orderSummary = {
      ...
      shipping:
        orderData.details?.shipping_cost ||
        orderData.order_detail?.shipping_cost ||
        0,
      tax:
        orderData.details?.tax_amount ||
        orderData.order_detail?.tax_amount ||
        0,
      ...
      case "cancelled":
        ...
        await Promise.all(
          order.order_items.map(async (item) => {
            ...
            const combination =
              await sequelize.models.VariantCombination.findByPk(
                item.combination_id,
                { transaction }
              );
