...
/**
 * Validation rules for vendor rejection.
 * Validates rejection reason when admin rejects a vendor application.
 * @type {Array<ValidationChain>} Array of express-validator validation chains
 * @property {ValidationChain} reason - Required, 10-1000 chars, trimmed
 * @returns {Array} Express validator middleware array for vendor rejection
 * @example
 * // Use in route:
 * router.patch('/vendors/:id/reject', rejectVendorValidation, validate, rejectVendor);
 */
...
/**
 * Validation rules for initializing vendor onboarding.
 * Validates business information required to create minimal vendor and store records.
 * @type {Array<ValidationChain>} Array of express-validator validation chains
 * @property {ValidationChain} business_name - Required, 2-100 chars, trimmed, unique
 * @property {ValidationChain} join_reason - Required, 10-1000 chars, trimmed
 * @property {ValidationChain} cac_number - Optional, valid CAC format
 * @property {ValidationChain} instagram_handle - Optional, valid handle format
 * @property {ValidationChain} facebook_handle - Optional, valid handle format
 * @property {ValidationChain} twitter_handle - Optional, valid handle format
 * @returns {Array} Express validator middleware array for vendor onboarding initialization
 * @example
 * // Use in route:
 * router.post('/vendors/initialize-onboarding', initializeOnboardingValidation, validate, initializeVendorOnboarding);
 */
const initializeOnboardingValidation = [
  // Store fields - minimal required for initialization
  body("business_name")
    .trim()
    .notEmpty()
    .withMessage("Business name is required")
...
/**
 * Validation rules for updating vendor profile.
 * Allows partial updates for user and store information.
 * @type {Array<ValidationChain>} Array of express-validator validation chains
 */
const updateVendorProfileValidation = [
  ...
  body("phone")
    ...
    .matches(/^\+234(70|80|81|90|91)[0-9]{8}$/)
    .withMessage("Phone number must be in the format +234[70|80|81|90|91]XXXXXXX"),
  ...
  body("business_name")
    ...
    .withMessage("Business name must be between 2 and 100 characters")
    .custom(async (value, { req }) => {
      // Find store associated with the logged-in vendor
      const vendor = await req.db.models.Vendor.findOne({ 
        where: { user_id: req.user.id },
        include: ['store'] 
      });
      
      if (!vendor || !vendor.store) {
        // Should be caught by controller, but safe to ignore here or fail
        return true; 
      }
