...
1. **Health Check Endpoint**: We tested the health check endpoint at `http://localhost:5000/health` and received the expected response:
   ```json
   {"status":"ok","timestamp":"2025-11-06T09:51:15.983Z"}
   ```

2. **Metrics Endpoint**: We tested the metrics endpoint at `http://localhost:5000/metrics` and confirmed that Prometheus metrics are being collected and exposed.
